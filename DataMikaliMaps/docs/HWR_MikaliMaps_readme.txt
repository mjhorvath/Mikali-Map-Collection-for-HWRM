...
to the mod on its Steam Workshop page, and then using the "Mods" section of the 
game launcher to activate the mod.
...
after the "-moddatapath" command line parameter. Multiple names should be 
separated by commas.

For instance: "-moddatapath DataPlayersPatch,DataMikaliMaps,DataTestMod".
...
Maps marked with a single asterisk (*) are are pseudo-randomly generated. I 
have provided five different versions of these maps, each with a different seed 
value. I have also provided a sixth version with a randomized seed. This means 
that each time you load the sixth map it will be completely randomized. This is 
not multiplayer-safe! In multiplayer against other human players a randomized 
seed will cause desyncs! Only play over the Internet using the five non-
randomized seeds. The randomized seeds should work okay in skirmish mode, 
however.

Maps marked with two asterisks (**) feature radioactive clouds (usually in the 
center of the map) that can destroy your ships if you fly into them. These maps 
are marked with a yellow triangle in the level thumbnail images.

Maps marked with three asterisks (***) feature indestructible hyperspace 
inhibitors. Hyperspacing will not be possible within a certain distance of the 
inhibitors.
